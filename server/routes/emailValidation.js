...
export async function sendVerificationCode(email) {
    ...
    const { insertId } = await User.findByIdAndUpdate({ _id: email }, { code })
    const verificationURL = `${process.env.BASE_SITE_URL}verify?token=${jwt.sign({ user_uuid, email, code, insertId }, process.env.TOKEN_SECRET)}`;
    await sendEmail({
      to: email,
      html:
